...
class CausalMaskedDiffWithDiT(torch.nn.Module):
    ...
    def forward(
        ...
        # Align h / mask to mel-feat length — token_len*ratio can differ by ±1
        # from feat_len due to separate extraction pipelines (e.g. Arabic data).
        max_feat = feat.shape[1]
        if h.shape[1] > max_feat:
            h = h[:, :max_feat, :]
            mask = mask[:, :max_feat]
        elif h.shape[1] < max_feat:
            pad_len = max_feat - h.shape[1]
...
if __name__ == '__main__':
    ...
    with open('./pretrained_models/CosyVoice3/cosyvoice3.yaml', 'r') as f:
