...
def load_wav(wav, target_sr, min_sr=16000):
    """Load audio from a path/file-like object *or* accept an in-memory tensor/ndarray.

    - If `wav` is a `torch.Tensor` or `np.ndarray`, it is treated as already-resampled
      waveform in samples (mono or multi-channel). We only ensure shape/dtype.
    - Otherwise we delegate to `torchaudio.load` (supports filepath / file-like / bytes).

    This avoids passing a raw Tensor into `torchaudio.load` (which triggers the
    torchcodec "create_from_tensor" path and may fail when the tensor is not
    a uint8 video-like buffer).
    """
    # Accept in-memory audio (torch tensor)
    if isinstance(wav, torch.Tensor):
        speech = wav
        # 1-D -> (1, T), 2-D assumed (C, T)
        if speech.dim() == 1:
    ...
    # Fallback: path / file-like object — use soundfile (torchaudio.load is
    # broken in v2.10+ where it ignores backend='soundfile' and requires torchcodec/FFmpeg)
    import soundfile as sf
    data, sample_rate = sf.read(wav, dtype='float32')
    speech = torch.from_numpy(data)
    if speech.dim() == 1:
        speech = speech.unsqueeze(0)  # (1, T)
    else:
        speech = speech.T  # soundfile returns (T, C), we need (C, T)
