...
def _prune_checkpoints(model_dir):
    """Prune older checkpoints to reduce disk growth.

        Controlled by env vars:
            - COSYVOICE_KEEP_STEP_CKPTS (default: 1)
      - COSYVOICE_KEEP_WHOLE_CKPTS (default: 1)
    """
    try:
        keep_step = int(os.environ.get('COSYVOICE_KEEP_STEP_CKPTS', '1'))
        keep_whole = int(os.environ.get('COSYVOICE_KEEP_WHOLE_CKPTS', '1'))
    except Exception:
        keep_step, keep_whole = 1, 1
    ...
    try:
        step_pts = _sort_by_mtime(glob.glob(os.path.join(model_dir, 'epoch_*_step_*.pt')))
        whole_pts = _sort_by_mtime(glob.glob(os.path.join(model_dir, 'epoch_*_whole.pt')))

        to_delete = []
        if len(step_pts) > keep_step:
            to_delete.extend(step_pts[:len(step_pts) - keep_step])
        if len(whole_pts) > keep_whole:
            to_delete.extend(whole_pts[:len(whole_pts) - keep_whole])

        for pt in to_delete:
            yml = re.sub(r'\.pt$', '.yaml', pt)
            try:
        ...
        if to_delete:
            logging.info('[CheckpointPrune] Removed {} old checkpoints from {}'.format(len(to_delete), model_dir))
    except Exception as e:
        logging.warning('[CheckpointPrune] Failed: {}'.format(e))
    ...
    if args.train_engine == 'torch_ddp':
        if torch.cuda.is_available():
            ...
            if world_size > 1:
                ...
                logging.info("Single-process run detected; skipping process group init")
        else:
            logging.warning("CUDA not available; skipping process group init")
    else:
    ...
    if args.train_engine == "torch_ddp":  # native pytorch ddp
        if torch.cuda.is_available():
            ...
            if dist.is_available() and dist.is_initialized() and world_size > 1:
                ...
                logging.info("Single-process torch_ddp mode; running without DDP wrapper")
        else:
            logging.warning("CUDA not available; running on CPU without DDP wrapper")
    else:
...
def cosyvoice_join(group_join, info_dict):
    if group_join is None or not dist.is_initialized():
        return False
    world_size = int(os.environ.get('WORLD_SIZE', 1))
    local_rank = int(os.environ.get('LOCAL_RANK', 0))
    rank = int(os.environ.get('RANK', 0))
...
def log_per_step(writer, info_dict):
    ...
    # TRAIN & CV, Shell log (stdout) - Enhanced with step counter
    if (info_dict['batch_idx'] + 1) % info_dict['log_interval'] == 0:
        ...
        log_str += f' || step {step} || rank {rank}'
        logging.debug(log_str)

        if rank == 0 and tag == "TRAIN" and 'progress_callback' in info_dict:
            try:
                callback = info_dict['progress_callback']
                callback(epoch, step, batch_idx, loss_dict, info_dict.get('lr', 0), info_dict.get('grad_norm', 0))
            except Exception:
                pass
