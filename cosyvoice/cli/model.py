...
class CosyVoiceModel:
    ...
    def load(self, llm_model, flow_model, hift_model):
        # NOTE: strict=False is required because llm.pt may contain LoRA-structured
        # keys (base_layer/lora_A/lora_B) from a previous training run, while the
        # model expects standard keys (q_proj.weight).  The Qwen2 attention weights
        # are already loaded via Qwen2ForCausalLM.from_pretrained() in __init__,
        # so missing attention keys are harmless.
