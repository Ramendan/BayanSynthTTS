...
class CosyVoiceFrontEnd:
    ...
    def _extract_speech_token(self, prompt_wav):
        ...
        # ensure feat has shape (batch=1, n_mels, time)
        if feat.dim() == 2:
    ...
    def frontend_sft(self, tts_text, spk_id):
        ...
        if spk_id not in self.spk2info:
            raise ValueError('speaker id "{}" not found in spk2info; no pre-registered speakers available.\n'
                             'Use zero-shot mode (upload a reference audio) or add a speaker via CosyVoice.add_zero_shot_spk()'.format(spk_id))
